...
def apply_lora( clip_model, encoder='both', position='all', params=['q', 'k', 'v'], r=16, alpha=16, dropout_rate=0.25, backbone='vit_b32'):
    list_lora_layers = []
            ...
            if i in indices:
                for name, submodule in block.named_children():
                    if isinstance(submodule, nn.MultiheadAttention):
                        new_multi_head_lora = PlainMultiheadAttentionLoRA( submodule, enable_lora=params, r=r, lora_alpha=alpha, dropout_rate=dropout_rate)
                        setattr(block, name, new_multi_head_lora)
                        list_lora_layers.append(new_multi_head_lora)

    if encoder == 'vision' or encoder == 'both':
        indices = INDEX_POSITIONS_VISION[backbone][position]
        vision_encoder = clip_model.image_encoder.transformer
